...
## 2026-09-16 — セッション10(続き): 通知の重複防止ロジック修正・本番メール送信の制約確認

**背景**
本番デプロイ後、価格アラート条件をすでに満たしている商品に後からアラートを設定しても、初回の通知メールが送信されないケースがあることが分かった。

**決定・修正内容**
- `price_below` アラートの重複通知防止ロジックが、直前の価格スナップショットの値だけを見て判定していたため、「アラートを作成する前から条件を満たす価格履歴がある」場合に、そのアラート自身は一度も通知していないにもかかわらず「既に通知済み」と誤判定していた。判定基準を「このアラートが、価格が閾値を上回ってから再び下回るまでの間に、既に通知を送っているか」という、アラート単位の状態に修正した。
- 修正後、ローカル環境で実際にメール送信(Gmail SMTP)まで含めて動作することを確認した。
- 本番(Render Free)で同じ手順を試したところ、`SMTPConnectTimeoutError` で送信に失敗することが判明した。調査の結果、RenderはFreeプランのWebサービスに対して2025年9月から外向きのSMTP通信(ポート25/465/587)をブロックしていることが分かった([Render公式changelog](https://render.com/changelog/free-web-services-will-no-longer-allow-outbound-traffic-to-smtp-ports))。アラート評価・重複防止・通知履歴の記録など、アプリ側のロジックはすべて正しく動作しており、SMTPによる送信経路のみがホスティング環境側の制約に該当する。

**確認結果**
バックエンドのテスト・lintが通ることを確認した上でデプロイ。本番ログで `SMTPConnectTimeoutError` を確認し、原因がアプリ側の実装ではなくRender Freeプランのネットワーク制約であることを切り分けた。対応方針(Renderの有料プラン移行、またはHTTPS経由のメールAPIへの切り替え)は [docs/deployment.md](deployment.md) の既知の制約として記録した。
