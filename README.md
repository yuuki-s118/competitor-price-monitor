...
選定理由は [docs/process-log.md](docs/process-log.md) を参照。データモデルは [docs/er-diagram.md](docs/er-diagram.md) を参照。
...
`collect-price` は手動トリガー用のエンドポイントで、これとは別に Celery beat が毎時、有効な監視対象商品すべてについて同じ処理を自動実行する(`celery-worker` / `celery-beat` コンテナ)。
